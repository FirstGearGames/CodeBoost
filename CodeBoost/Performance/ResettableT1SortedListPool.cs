...
/// <summary>
/// A pool for a SortedList whose values are resettable.
/// </summary>
public static class ResettableT1SortedListPool<T0, T1> where T1 : IPoolResettable, new()
{
    ...
    /// <summary>
    /// Resets each value in the SortedList by invoking <see cref="IPoolResettable.OnReturn"/>; the SortedList itself is not returned to the pool.
    /// </summary>
    /// <param name="value">SortedList whose values to reset. Null values are ignored.</param>
    ...
    /// <summary>
    /// Resets each value in the SortedList via <see cref="Reset"/> and returns the SortedList to the pool.
    /// </summary>
