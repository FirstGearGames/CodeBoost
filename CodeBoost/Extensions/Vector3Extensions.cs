...
public static class Vector3Extensions
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Vector3 Quantize(this Vector3 value, float accuracy)
    {
        return new(value.X.Quantize(accuracy), value.Y.Quantize(accuracy), value.Z.Quantize(accuracy));
    }
    ...
    {
        return new(value.X.QuantizeUnsafe(accuracy), value.Y.QuantizeUnsafe(accuracy), value.Z.QuantizeUnsafe(accuracy));
    }

    //xml 
    public static bool IsWithinAccuracy(this Vector3 value, Vector3 otherValue, float accuracy) 
    {
        return Math.Abs(value.X) <= accuracy && Math.Abs(value.Y) <= accuracy && Math.Abs(value.Z) <= accuracy;
    }
}
